...
addChildBtn.addEventListener("click", () => {
  const d = slotData[currentSlot];
  if (!d?.itemID) return alert("Assign a main item first before adding children.");
  d.children = d.children || [];
  d.children.push({ itemID: "", colorHue: 0, colorSaturation: 0, scale: 0 });
  saveSlot();
  alert("Child added — edit it directly in the JSON.");
});
...
adminOverrideBtn.addEventListener("click", () => {
  ...
  saveSlot();
  alert("Override applied to sliders!");
});
...
addNewItemBtn.addEventListener("click", () => {
  ...
  if (!name || !id) return alert("Please provide both name & ID");
  if (items.some(x => x.id === id)) return alert("ID already exists");
