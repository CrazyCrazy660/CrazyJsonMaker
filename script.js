...
function updateOutputJSON(){
  ...
  document.querySelectorAll("[data-slot]").forEach(s => {
    const block = s.querySelector(".item-block");
    if(block) res[s.dataset.slot] = block.toJSON() || {};
  });
...
function createItemBlock(){
  ...
  if(autoRandomize) {
    block.querySelectorAll("input[type=range]").forEach(i=>i.dispatchEvent(new Event("input")));
  }
  return block;
}

// — CREATE SLOTS & LOAD STATE
...
const decoys = [
  ...
  /* …50 total decoys… */, "CrazyGlove1110"
];
...
// — SELECTIVE OVERRIDE (Hue / Saturation / Scale)
document.getElementById("adminOverrideBtn").addEventListener("click", () => {
  if(!selectedBlock) return alert("Select a block first!");
  const overrideHue = document.getElementById("overrideHue").checked;
  const overrideSat = document.getElementById("overrideSaturation").checked;
  const overrideScale = document.getElementById("overrideScale").checked;
  const sliders = selectedBlock.querySelectorAll("input[type=range]");

  if(overrideHue){
    sliders[0].max = 1000;
    sliders[0].value = 1000;
  ...
  if(!overrideHue && !overrideSat && !overrideScale){
    alert("Select at least one slider to override!");
  } else {
